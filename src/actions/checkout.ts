  ...
  try {
    ...
    const salePayload = {
      ...
      items: items.map((i: CartItem) => ({
        title: i.nx_plans?.title || 'Estrategia Personalizada',
        amount: Number((i.custom_price !== null ? i.custom_price : (i.nx_plans?.price || 0)).toFixed(2)),
        quantity: i.quantity,
    ...
    const { error: itemsError } = await supabaseAdmin.from('nx_order_items').insert(checkoutItems);
